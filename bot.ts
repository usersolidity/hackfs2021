...
const FILE_TEXTILE =
  "https://slate.textile.io/ipfs/bafybeib35oktwvz3iskxbz7vz4ukow7amnmaldke6w2y2rvfpad54k335a";
  ...
  try {
    if (msg.content === "!nft.storage") {
      ...
      const metadata = await client.store({
        ...
        image: new File(["./images/1.png"], 'mynft.jpg', { type: 'image/jpg' })
      })
      
      const msgNFTEmbed = new MessageEmbed()
    ...
    if (msg.content === "/join") {
      ...
      if (msg.member.voice.channel) {
        ...
        const dispatcher = connection.play(
          ytdl(URL_VIDEO_ETHGLOBAL, { filter: "audioonly" })
        );
      } else {
        msg.reply(
          "You need to join a voice channel :) and wait a sec to start listening!"
        );
      }
    }
    if (msg.content === "!covalent") {
      const result = await covalentjs.classA.getTokenBalancesForAddress(
        80001,
        MY_ADDRESS,
        { nft: true }
      );
      ...
      for (let index = 0; index < last; index++) {
        const covalentEmbed = new MessageEmbed()
          ...
          .addFields(
            { name: "Address", value: `${result.data.address}`, inline: true },
            {
              name: "Updated at",
          ...
          .setTimestamp()
          .setFooter(
            "Network: " + BOT_NAME_FOOTER,
            result.data.items[index].logo_url
          );
        msg.channel.send(covalentEmbed);
      }
    }
    if (msg.content === "!web3storage") {
      const storage = new Web3Storage({ token: process.env.API_WEBSTORAGE });
      const info = await storage.status(
      ...
      for (const file of files) {
        const msgFileEmbed = new MessageEmbed()
          .setColor(EMBED_FILE)
          .setDescription(BOT_NAME + " web3storage ")
          .setURL(URL_BOT)
    ...
    if (msg.content === "!we3storageupload") {
      ...
      console.log(storage);
      const files = [];
      const pathFiles = await getFilesFromPath("./images");
    ...
    if (msg.content === "!templatepost") {
      const rawResponse = await fetchJson.post("https://httpbin.org/post", {
        method: "POST",
        headers: {
